...
p <- ggplot(dt_plot, aes(x = random_index, y = -log10(FDR), label = genelabel, color = factor(hit), alpha = -log10(FDR))) + geom_point(size = 0.01) + ggrepel::geom_text_repel(size = 2, max.overlaps = 100) + facet_wrap(~screen_label, ncol = 1) + theme_minimal()
p <- p + scale_color_manual(values = hitcolors)
p <- p + theme(axis.text = element_text(size = 5), axis.title.x = element_blank(), axis.title.y = element_text(size = 7), legend.position = "none")
p

s3save_ggplot(ggp = p, 
							object_s3uri = file.path(s3outpath, "hitsummary", "Fig 1 scatterview.pdf"),
							s3bucket = s3bucket, width = 7, height = 10, units = "in")
s3save_ggplot(ggp = p, 
							object_s3uri = file.path(s3outpath, "hitsummary", "Fig 1 scatterview.jpg"),
							s3bucket = s3bucket, device = "jpeg", width = 7, height = 10, units = "in", dpi = 320, bg = "white")
```
