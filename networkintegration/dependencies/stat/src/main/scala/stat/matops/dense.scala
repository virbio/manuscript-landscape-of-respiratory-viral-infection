...
object DenseVecOps extends VecOps[Vec[Double]] {
  ...
  def classTag: scala.reflect.ClassTag[org.saddle.Vec[Double]] =
    implicitly[ClassTag[T]]
  def index(t: org.saddle.Vec[Double], i: Int): Int = i
  def nonZeroLength(t: org.saddle.Vec[Double]): Int = t.length
...
object DenseMatOps extends MatOps[Mat[Double]] with LinearMap[Mat[Double]] {
  ...
  def colMeans(t: org.saddle.Mat[Double]): org.saddle.Vec[Double] =
    t.colSums / t.numRows
  ...
  def mDiagFromRight(t: T, v: Vec[Double]): T = t.mDiagFromRight(v)
  ...
  def mapValues(t: T, f: Double => Double) = t.map(f)
  def rowSums(t: T) = t.rowSums

  def squaredEuclideanDistanceMatrix(t: T) =
    stat.util.Distances.squaredEuclideanDistance(t)
}
