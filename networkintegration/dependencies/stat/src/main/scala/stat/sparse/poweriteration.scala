...
object Eigen {
  def eigenDecompositionSymmetric[T: LinearMap](
    ...
    def loop(
        t: T,
        k: Int,
        acc: List[(Vec[Double], Double)]
    ): List[(Vec[Double], Double)] =
      if (k == 0) acc
      else {
...
object PowerIteration extends StrictLogging {
  ...
  def iteration[T: LinearMap](
      ...
      min: Int,
      epsilon: Double
  ): Option[(Vec[Double], Double, Double)] = {
    val t =
  ...
  def from[T: LinearMap](
    ...
    def loop(
        start: (Vec[Double], Double, Double)
    ): Stream[(Vec[Double], Double, Double)] =
      start #:: loop({
        val (ve, _, _) = start
        ...
        logger
          .debug(s"Power iteration error: $co, eval: $nva, evec: $nva")
