...
object nsplcanvastest {
  ...
  def bind(n: Node): Unit = {
    ...
    val p6 =
      rasterplot(rasterFromStream(z3.iterator, 30, 30, MinMaxImpl(0.0, 1.0)))(
    ...
    val xyzp = xyzplot(
      ...
      (cubeVertex, List(point3D(size = 0.1d, colorCol = 2)), NotInLegend)
    )(cameraTarget = Math3D.Vec3(50f, 50f, 50f))
    ...
    val (canvas, update) =
      render(
        gallery,
        800,
        hover = Some((x,y) => println(x)),
        click = Some((x,y) => println(x)),
        selection = Some(println)
      )
    n.appendChild(canvas)

    n.asInstanceOf[HTMLHtmlElement].onclick = { _ =>
