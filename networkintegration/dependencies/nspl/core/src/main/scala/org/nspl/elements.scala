...
object ElemOption {
  implicit def renderer[T1 <: Renderable[T1], R <: RenderingContext[R]](
      implicit
      r1: Renderer[T1, R]
...
case class ShapeElem(
  ...
  def withDescriptor(d: Descriptor) = copy(descriptor = d)
...
object TextBox {
  def apply[F: FC](
    ...
    TextBox(
      layout = TextLayout(width, text, fontSize),
      loc = loc,
      color = color,
      tx = tx,
      descriptor = descriptor,
