  ...
  def extremities(p0: Point) = {
    ...
    val t1x = (
      math.sqrt(
        ...
        + 2 * p1.x
        - p2.x
    ) / (-1 * p0.x + 3 * p1.x - 3 * p2.x + p3.x)

    val t2x = -1 * (math.sqrt(
      -1 * p0.x * p2.x + p0.x * p3.x + p1.x * p1.x - p1.x * p2.x - p1.x * p3.x + p2.x * p2.x)
      + p0.x
      - 2 * p1.x
      + p2.x) / (-1 * p0.x + 3 * p1.x - 3 * p2.x + p3.x)

    val t1y = (math.sqrt(
      -1 * p0.y * p2.y + p0.y * p3.y + p1.y * p1.y - p1.y * p2.y - p1.y * p3.y + p2.y * p2.y)
      - p0.y
      + 2 * p1.y
      - p2.y) / (-1 * p0.y + 3 * p1.y - 3 * p2.y + p3.y)
...
case class Path(path: Seq[PathOperation], currentTransform: AffineTransform)
    extends Shape {
  def bounds: Bounds = {
    val extrema = path.sliding(2).toList.flatMap { pair =>
      val first = pair(0)
      val second = pair(1)
  ...
  def resetTransform(tx: Bounds => AffineTransform) =
    this.copy(currentTransform = tx(bounds))
}
