...
case class RelativeToFirstAnchor(x: Double, y: Double) extends Layout {
  def apply[F: FC](s: Seq[Bounds]) = {
    if (s.isEmpty) Nil
    else {
      val first = s.head
      val firstAnchorX = first.anchor.map(_.x).getOrElse(first.centerX)
      val firstAnchorY = first.anchor.map(_.y).getOrElse(first.centerY)
      first +: s.drop(1).map { b =>
        val deltaX = (firstAnchorX - b.anchor.map(_.x).getOrElse(b.centerX))
        val deltaY = (firstAnchorY - b.anchor.map(_.y).getOrElse(b.centerY))
        Bounds(
          first.x + deltaX + x,
          first.y + deltaY + y,
          b.w,
          b.h,
          b.anchor.map(_.translate(b.x - deltaX - x, b.y - deltaY - y))
        )
      }
    }
  }
...
case class TableLayout(
    columns: Int,
    horizontalGap: RelFontSize = 1d fts,
    verticalGap: RelFontSize = 1d fts
) extends Layout {
...
case class ColumnLayout(
    numRows: Int,
    horizontalGap: RelFontSize = 2.5 fts,
    verticalGap: RelFontSize = 2.5 fts
) extends Layout {
  def apply[F: FC](s: Seq[Bounds]) = {
    if (s.isEmpty) s
    else
